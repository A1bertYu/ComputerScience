...
##第22章 图的基本算法  
搜索一个图是有序地沿着图的边访问所有顶点。图的搜索算法可以使我们发现图的很多结构信息。图的搜索技术是图算法领域的核心。  
图在计算机中两种最主要的表示法：邻接表表示和邻接矩阵表示。  
###22.1 图的表示  
G=(V, E)，当|E|远小于|V|^2时，通常采用邻接表表示法；当二者接近时，则往往采用邻接矩阵表示法。  
邻接表表示法，共包含|V|个列表，对于顶点u的列表，用Adj(u)表示。若G是有向图，则所有邻接表的长度之和为|E|；若G是无向图，则所有邻接表的长度之和为|2E|.占用空间为Theta(E+V)  
邻接矩阵表示法，为|V|x|V|的矩阵，占用空间为Theta(V^2)。当表示无向图时，邻接矩阵是对称的。  
由上所述，不难得知二者所占空间情况，这也是为什么通过判断|E|与|V|^2的关系，来选用合适的表示法。	 
...
BFS适用于有向图和无向图。
	...
	Q is empty
	Q.push(s)
	while Q is not empty /* 可用循环不变式证明Q中一直为灰色顶点 */
		u = Q.top();
		Q.pop()
		for v in Adj(u)
			if color(v) == WHITE
				color(v) = GRAY /* 灰色表示并未对该顶点的邻接表进行完全搜索 */
				d(v) = d(u) + 1
				p(v) = u
				Q.push(v)
		color(u) = BLACK/* 标记为黑色表示该顶点的邻接表中的所有点已经被发现（已被完全搜索） */
BFS之所以如此命名，是因为它总是从广度上扩展已发现顶点和未发现顶点之间的边界（it expands the frontier between discovered and
undiscovered vertices uniformly across the breadth of the frontier）。从上面代码，我们可以看出，未被发现的顶点（白色），才会被加入队列（加入时为灰色），以便继续探索。   
BFS能生成一颗根为s，且包括所有s的可达顶点的广度优先树，且对于s的任意可达顶点v，优先树中s到v的路径，对应于图中s到v的一条最短路径（包含边数最少）。    
 引理22.1，注意证明时已有假定条件边是(u, v)，也就是已经指定了方向（即使是无向图，这样写也表示是u到v的边），是u到v。当然，你也可以用(v,u)得到类似结论，结论不会因为u,v的方向而变化。你可以想象一下s,u,v分别是三角形的顶点的情况。  
引理22.2，对伪代码使用归纳法的证明方式，值得学习。最初队列只有源顶点s时，此时算初始情况；对于每一个顶点，其长度的计算在书中源码的第15行（P325），因此，若假定d[u]满足定理中条件，那么d[v]也就可以推知满足。    
引理22.3，也是对伪代码采用归纳的证明方式。最初始情况是队列中只有源顶点。对于队列中顶点性质的证明，就需要考虑两个方面的归纳证明，即插入和去掉一个顶点。插入的情况，证明时，要借助已被弹出的u，以及第15行代码。定理22.2和22.3可以适当利用代码的特点，比如u和v，类似于归纳法中n=k和n=k+1的情况。  
###22.3 深度优先搜索  
...
在DFS中，对于最新发现的顶点，若它还有以此为起点而未探测的边，就沿此边继续探测下去。  
